...
def fast_gradient_attack(logits: torch.Tensor, x: torch.Tensor, y: torch.Tensor, epsilon: float, norm: str = "2",
    ...
    loss.backward()
    if norm == "1":
        B,C,N,N = x.shape[0],x.shape[1],x.shape[2],x.shape[3]
        data = torch.abs(x.grad)
        norm1 = torch.sum(data,dim = (1,2,3)) # one batch one norm
        norm1 = norm2.reshape((B,1))
        norm1 = norm2 * torch.ones((B,C*N*N))
        norm1 = norm2.reshape((B,C,N,N))
        x_pert = x + epsilon * data/norm1
        x_pert = torch.clamp(x_pert,0,1.0)

    if norm == "2":
        B,C,N,N = x.shape[0],x.shape[1],x.shape[2],x.shape[3]
        data = x.grad
        norm2 = torch.sum(data**2,dim = (1,2,3)).sqrt() # one batch one norm
        norm2 = norm2.reshape((B,1))
        norm2 = norm2 * torch.ones((B,C*N*N))
        norm2 = norm2.reshape((B,C,N,N))
        x_pert = x + epsilon * data/norm2
        x_pert = torch.clamp(x_pert,0,1.0)
