...
class SmoothClassifier(nn.Module):
    ...
    def certify(self, inputs: torch.Tensor, n0: int, num_samples: int, alpha: float, batch_size: int) -> Tuple[int,
        ...
        ##########################################################
        # YOUR CODE HERE
        class4class = self._sample_noise_predictions(inputs, n0, batch_size)
        top_class = torch.argmax(class4class)
        class4pro = self._sample_noise_predictions(inputs, num_samples, batch_size)
        numofclass = class4pro[top_class]
        p_A_lower_bound = lower_confidence_bound(numofclass, num_samples, alpha)
        ##########################################################
    ...
    def predict(self, inputs: torch.tensor, num_samples: int, alpha: float, batch_size: int) -> int:
        ...
        ##########################################################
        # YOUR CODE HERE
        sort, indices = torch.sort(class_counts, descending = True)
        count1, count2 = sort[0], sort[1]
        topclass = indices[0]
        if binom_test(count1, count1 + count2, p=0.5) > alpha == True:
    ...
    def _sample_noise_predictions(self, inputs: torch.tensor, num_samples: int, batch_size: int) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            for it in range(ceil(num_samples / batch_size)):
                ...
                pert_inputs = inputs.repeat(this_batch_size,1,1,1)
                noise = torch.distributions.Normal(0,self.sigma).sample(pert_inputs.shape)
                pert_inputs = pert_inputs + noise
                pert_inputs = torch.clamp(pert_inputs,0,1.0)
                logits = self.base_classifier(pert_inputs)
